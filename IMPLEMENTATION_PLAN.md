...
> **Статус: усі 14 кроків виконано.** Що з'ясувалося під час реалізації й розійшлося з планом —
> у розділі «Підсумок реалізації» наприкінці документа.
...
Стан на кінець робіт: `dotnet build Unmatched.sln` — 0 помилок; `dotnet test Unmatched.sln` — 160 тестів,
усі зелені (Catalog 17, Statistics 15, Match 80, Player 1, новий `Unmatched.Tests` 47).
Усі маршрути застосунку віддають 200 у контейнері.

### Що знайшлося по дорозі і не було в плані

1. **`IDataInitializer` — мертвий код.** І реалізація (`DataInitializer.cs`), і реєстрація були
   закоментовані ще з часів розділення на мікросервіси: сторінка `/initialize` не могла працювати
   в принципі. Секцію «Initialize default data» прибрано з Settings, посилання на
   `Unmatched.Initializer` — з `Unmatched.UI.BlazorServer.csproj`. Каталог заповнюють EF-міграції.
2. **Віллени й мініони не були прив'язані до доповнень.** `ExpansionId` у них лишався `NULL`, бо базу
   відновлювали з `.bacpac` уже після того, як `AddKnownExpansions`/`AddMissingCatalogContent`
   позначилися застосованими. Додано ідемпотентну міграцію `BackfillVillainMinionExpansions`, яка
   переприкладає **ті самі** прив'язки, що вже задокументовані в наявних скриптах — нічого не
   вигадано. Результат: 4 віллени й 12 мініонів розкладено по двох коробках.
3. **Відсутній `@using` у Razor не ламає збірку.** Компонент мовчки рендериться як невідомий
   HTML-тег — саме так `MatchList` спершу «зник» зі сторінки героя. Простори імен компонентів тепер
   імпортуються глобально в `_Imports.razor`, щоб цей клас помилок не повторювався.
4. **`RatingChangeDto.RatingDelta` містить накопичений рейтинг**, а не дельту (попри назву). Графік
   2a малює серію як є; поле не перейменовували, щоб не чіпати контракт Match-сервісу.

### Свідомі відхилення від макета

| Місце | Відхилення | Причина |
|---|---|---|
| 1c, підзаголовок | Немає «updated 4 min ago» | `HeroStats.ModifiedAt` не доходить до UI; заради косметичного підпису контракт трьох шарів не чіпали |
| 1c, лівий край рядка | Медальні кольори для топ-3, червоний для від'ємних очок, решта нейтральні | Так на скріншоті `1c`; у дизайн-системі той самий рядок показано з win/loss-краєм — узято екран як авторитет |
| 2a, банер | Круглий токен замість 3:4 key art | Рішення #1 |
| 2a, шапка | Немає аватарки користувача праворуч | Застосунок не per-user |
| 5a, рядок мініона | Немає `×N` | Рішення #6: кількість рахується як кількість рядків за `ExpansionId` |
| 4b | Є компактний перемикач ходу (Turn) | Рішення #9: поле потрібне сортуванню бійців і генерації турнірних пар |
| 6a | Кнопка Export вимкнена | Рішення #8 |

### Додано понад план

- `Unmatched.Tests` — тестовий проєкт для спільної бібліотеки (метрики матчлогу, матчапи, фільтр,
  форма матчу за режимом, короткі назви доповнень).
- `run-ui-local.ps1` + `.claude/launch.json` — запуск UI на хості проти вже піднятих у Docker
  сервісів. Обмеження задокументоване у самому скрипті: без рантайму .NET 9 застосунок їде на
  roll-forward до .NET 10, де `blazor.server.js` не віддається, тож інтеракції треба перевіряти
  в контейнері.
- Прибрано мертві залежності: Chart.js і плагін datalabels (усі графіки тепер власний SVG),
  вендорований `open-iconic`, `Unmatched.otf`, `style.css`, `syncfusion-overrides.css`.
