...
## Revision 3 — adding a match

| # | Change | Ref |
|---|---|---|
| 11 | `Add match` opens a **New match** modal: *Manual entry* or *Draft session* (never called a quiz) | `12a` |
| 12 | **Draft session** — five steps: players & teams → map bans (N+1 pool) → hero ban + snake pick (N×3 pool) → final pick → turn-order roll, or a villain step in co-op → prefilled sheet | `12b` |
| 13 | The match sheet is **one component** reused by Add match, Edit match and step 5 of the draft | `14a` |
| 14 | Participant plate is now a **180px vertical card** (art → hero → player → HP/Cards/Sidekick steppers); minions are 132px cards in a column right of the villain | `14a` |
| 15 | **No Tournament / Stage on the sheet** — that slot is a `Ranked / Casual` segmented control; tournament and stage are attached from the bracket only | `14a` |
| 16 | Turn order is changed **only by dragging a card by the handle above its art**: 1-2-3 in 1v1/FFA, alternating teams in 2v2, **none at all in co-op** | `14a` |
...
- `MatchSheetCards.dc.html` — the match-sheet component itself (Add match, Edit match, draft step 5);
  open it directly or via turn `14a` of the prototype
